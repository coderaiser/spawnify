(function() {
    ...
    function onMessage(command, options, emitter) {
        ...
        if (isCD || isCDWin && WIN || isVolume) {
            ...
            onCD(command, dir, options, emitter);
        }
    ...
    function setListeners(child, emitter) {
        Util.check(arguments, ['child', 'callback']);
        ...
        child.on('error', function(error) {
            ...
            if (error) {
                ...
                error.message   = addNewLine(message);
            }
    ...
    function onCD(command, currDir, options, emitter) {
        var wasError,
            CD              = 'cd ',
        ...
        emitter.on('close', function() {
           if (!wasError) {
                options.cwd = paramDir;
                emitter.emit('cd', paramDir);
           }
        });
